...
export async function getChats(limit) {
  ...
  let query = supabase
    .from('chats')
    .select('*')
    .or(`user_id.eq.${user.id},user_id.eq.telegram`)
...
export async function getChatMessages(chatId) {
  ...
  if (!chat || (chat.user_id !== user.id && chat.user_id !== 'telegram')) {
    return [];
  }
...
export async function createChat(id, title = 'New Chat') {
  const user = await requireAuth();
  return await dbCreateChat({ id, userId: user.id, title });
}
...
export async function getNotifications(limit = 25, offset = 0) {
  await requireAuth();
  const rows = await dbGetNotifications(limit, offset);
...
export async function getChatData(chatId) {
  ...
  const chat = await getChatById(chatId);
  if (!chat) return null;
  if (chat.user_id !== user.id && chat.user_id !== 'telegram') return null;
  return {
    ...chat,
    workspace: null,
  };
...
export async function createChatWorkspace(repo, branch) {
  ...
  const featureBranch = `rawclaw/new-chat-${shortId}`;
